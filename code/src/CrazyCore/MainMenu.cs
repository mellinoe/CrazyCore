...
namespace CrazyCore
{
    public class MainMenu : Behavior
    {
        ...
        {
            float renderQuality = CrazyCorePreferences.Instance.RenderQuality;
            if (ImGui.DragFloat("Render Quality", ref renderQuality, 0.3f, 1.0f, 0.05f))
            {
                renderQuality = MathUtil.Clamp(renderQuality, 0.3f, 1.0f);
                CrazyCorePreferences.Instance.RenderQuality = renderQuality;
            ...
            int currentIndex = (int)CrazyCorePreferences.Instance.WindowStatePreference;
            if (ImGui.Combo("Display Mode", ref currentIndex, _displayModeOptions))
            {
                var newState = (InitialWindowStatePreference)currentIndex;
                if (CrazyCorePreferences.Instance.WindowStatePreference != newState)
                {
                    CrazyCorePreferences.Instance.WindowStatePreference = newState;
                    _gs.Context.Window.WindowState = GraphicsPreferencesUtil.MapPreferencesState(newState);
                }

            }

            currentIndex = (int)CrazyCorePreferences.Instance.BackEndPreference;
            if (ImGui.Combo("Graphics Backend", ref currentIndex, _graphicsBackEndOptions))
            {
                ...
                if (CrazyCorePreferences.Instance.BackEndPreference != newState)
                {
                    CrazyCorePreferences.Instance.BackEndPreference = newState;
                }
            }
            if (ImGui.IsLastItemHovered())
            {
